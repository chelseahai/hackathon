...
window.PatternResults=(()=>{
  ...
  function build(kind,measurements=window.MathDressMeasurements?.current || {}){
    const b=BodyBlock.draftBody({bust:84,backLength:38,...measurements});let pieces;
    ...
    if(kind==='dress')pieces=PrincessDress.draftPrincessDress(measurements).panels.map((p,i)=>piece(p.name,p.outline,p.notches,widthCenter(p.outline,0),i===3));
