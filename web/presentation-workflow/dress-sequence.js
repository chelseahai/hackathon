...
window.buildDressSequence=(input=MathDressMeasurements.current)=>{
  const V=(x,y)=>({x,y}),move=(ps,x=0,y=0)=>ps.map(p=>V(p.x+x,p.y+y)),fmt=n=>Number(n.toFixed(2));
  const b=BodyBlock.draftBody(input),s=SkirtBlock.draftSkirt({...input,skirtLength:50}),d=PrincessDress.draftPrincessDress(input),p=d.params,lift=-b.cfHem.y;
  ...
  phase('Back · waist and hip',`Set the waist to [(${fmt(p.waist)} + ${fmt(p.waistEase)}) / 4 − 0.75 = ${fmt(d.backWaist)} cm] and hip to [(${fmt(p.hip)} + ${fmt(p.hipEase)}) / 4 − 1 = ${fmt(d.backHip)} cm]. Allocate [${fmt(d.backDart)} cm] to the princess intake; the remaining reduction shapes the side.`,785,807,[[4,[0,2,4]]]);
  ...
  phase('Front · align the waist',`Lift the front bodice [${fmt(b.frontNeckWidth)} / 2 = ${fmt(lift)} cm] to bring its dropped waist to y = 0. Align the front skirt to the same waist, hip and hem levels as the completed back.`,766,780,[[1,[1,2,4]]]);
  ...
  phase('Front · waist and hip',`Set the waist to [(${fmt(p.waist)} + ${fmt(p.waistEase)}) / 4 + 0.75 = ${fmt(d.frontWaist)} cm] and hip to [(${fmt(p.hip)} + ${fmt(p.hipEase)}) / 4 + 1 = ${fmt(d.frontHip)} cm]. Allocate [${fmt(d.frontDart)} cm] to the princess intake.`,785,807,[[4,[1,3,5]]]);
...
window.DressSequence=window.buildDressSequence();
