...
function createServer({webRoot=path.resolve(__dirname,'../..'),analyzeFn=analyze,hasEnvKey=Boolean(process.env.GEMINI_API_KEY)}={}){
  ...
  return http.createServer(async(req,res)=>{
    try{
      ...
      if(pathname.startsWith('/api/')){
        ...
        if(pathname!=='/api/measurements/analyze'||req.method!=='POST')return json(res,404,{success:false,error:'Unknown endpoint.'});
        ...
        let length=0,chunks=[];for await(const chunk of req){length+=chunk.length;if(length>24*1024*1024){json(res,413,{success:false,error:'The photos are too large. Use files under 8 MB each.'});return;}chunks.push(chunk);}
